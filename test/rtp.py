...
class RTPConnection:
	...
	def addToSendBuffer(self, data):
		...
		for d in range(len(dataSegments)):
			source_port = int(self.socket_port)
...
class RTPSocket:
	...
	def accept(self):
		...
		if len(self.SYNqueue) == 0: # no SYN bits recevied
			...
			conn.socket_port = self.socket_port
	...
	def send(self):
		...
		"""
		print "STARTED TO SEND"

		while True:
			for key in self.connections:
				c = self.connections[key]
				if c.hasDataInBuffer():
					sendBuffer = c.sendBuffer
					addr = c.dst_addr
					...
					self.nextseqnum = 0
					while 1:
						self.N = math.floor(min(c.rwnd, c.cwnd) / RTPPacket.MSS)
						while (self.nextseqnum < self.base + self.N) and self.nextseqnum < len(sendBuffer):	
							#print "Sending packet, self.N = " + str(self.N)
							packetToSend = sendBuffer[self.nextseqnum]
							...
							if(self.base == self.nextseqnum):
								if t != None: #is there is a timer running, stop it
									t.cancel()
								t = threading.Timer(RTPPacket.RTT, self.timeout, [addr, sendBuffer])
								t.start()
							self.nextseqnum += 1
							self.N = math.floor(min(c.rwnd, c.cwnd) / RTPPacket.MSS)
						...
						if self.ackList[addr] != []:
							...
							if c.cwnd < c.ssthresh:
								with self.connLock:
									c.cwnd = c.cwnd + RTPPacket.MSS
							else:
								with self.connLock:                             #self.connections[addr].cwnd < self.connections[addr].rwnd:
									c.cwnd = c.cwnd + RTPPacket.MSS*(RTPPacket.MSS/c.cwnd)

							print "Received ACK"
							print "cwnd: " + str(c.cwnd)
							print "rwnd: " + str(c.rwnd)
							print "N: " + str(self.N)

							header = packet.header
							self.base = header.acknum + 1
							for i in range(0, self.base): #cumulative ACK
								sendBuffer[i].isACKED = True
							if(self.base == self.nextseqnum):
								t.cancel()
							else:
								if t != None:
	...
	def timeout(self, addr, sendBuffer):
		""" 
